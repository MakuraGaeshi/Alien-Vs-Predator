...
namespace RRYautja
{
    ...
    public class Comp_Yautja : ThingComp
    {
        ...
        public override void PostExposeData()
        {
            ...
            Scribe_Deep.Look<Corpse>(ref this.markCorpse, "markCorpse", false);
            Scribe_Deep.Look<Hediff>(ref this.marked, "marked", null, false);
            Scribe_Values.Look<string>(ref this.markedDef, "markedDef", null, false);
            ...
            if (base.parent.IsHashIntervalTick(30)&& base.parent != null && base.parent is Pawn pawn)
            {
                if (pawn.records.GetAsInt(RecordDefOf.Kills) > TotalkillsRecord)
                {
                    ...
                    if (pawn.LastAttackedTarget != null && pawn.LastAttackedTarget.Thing is Pawn other && !pawn.Dead)
                    {
                        ...
                        markedDef = blooded.def.stages[0].label;

                        HediffDef marked = DefDatabase<HediffDef>.GetNamed(markedDef);
                        //withComps.comps.Add();
                        if (unblooded==null)
                        {
