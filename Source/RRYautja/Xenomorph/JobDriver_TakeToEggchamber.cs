...
namespace RimWorld
{
    ...
    public class JobDriver_TakeToEggchamber : JobDriver
    {
        ...
        protected Thing DropEgg
        {
            get
            {
                return (Thing)this.job.GetTarget(TargetIndex.B).Thing;
            }
        }
        ...
        public override bool TryMakePreToilReservations(bool errorOnFailed)
        {
            ...
            if (pawn.Reserve(target, job, 1, -1, null, errorOnFailed))
            {
                ...
                target = this.DropEgg;
                job = this.job;
                int sleepingSlotsCount = this.DropEgg.SleepingSlotsCount;
                int stackCount = 0;
        ...
        protected override IEnumerable<Toil> MakeNewToils()
        {
            ...
            this.FailOn(delegate ()
            {
                ...
                {
                    if (!this.DropEgg.ForPrisoners)
                    {
                        return true;
                    }
                }
                else if (this.DropEgg.ForPrisoners != this.Takee.IsPrisoner)
                {
                    return true;
                }
                return false;
            ...
            {
                if (this.job.def.makeTargetPrisoner && this.Takee.ownership.OwnedBed == this.DropEgg && this.Takee.Position != RestUtility.GetBedSleepingSlotPosFor(this.Takee, this.DropEgg))
                {
                    this.Takee.ownership.UnclaimBed();
                }
            });
            yield return Toils_Goto.GotoThing(TargetIndex.A, PathEndMode.ClosestTouch).FailOnDespawnedNullOrForbidden(TargetIndex.A).FailOnDespawnedNullOrForbidden(TargetIndex.B).FailOn(() => this.job.def == JobDefOf.Arrest && !this.Takee.CanBeArrestedBy(this.pawn)).FailOn(() => !this.pawn.CanReach(this.DropEgg, PathEndMode.OnCell, Danger.Deadly, false, TraverseMode.ByPawn)).FailOn(() => this.job.def == JobDefOf.Rescue && !this.Takee.Downed).FailOnSomeonePhysicallyInteracting(TargetIndex.A);
            ...
            yield return new Toil
            {
                initAction = delegate ()
                {
                    IntVec3 position = this.DropEgg.Position;
                    Thing thing;
                    this.pawn.carryTracker.TryDropCarriedThing(position, ThingPlaceMode.Direct, out thing, null);
                    if (!this.DropEgg.Destroyed)
                    {
                    //    this.Takee.jobs.Notify_TuckedIntoBed(this.DropEgg);
