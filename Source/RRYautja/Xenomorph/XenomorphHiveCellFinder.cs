...
namespace RimWorld
{
    ...
    public static class XenomorphHiveCellFinder
    {
        ...
        {
            XenomorphHiveCellFinder.CalculateLocationCandidates(map);
            ...
            cell = CellFinder.FindNoWipeSpawnLocNear(locationCandidate.cell, map, XenomorphDefOf.RRY_Xenomorph_Hive, Rot4.North, 2, (IntVec3 x) => XenomorphHiveCellFinder.GetScoreAt(x, map) > 0f && x.GetFirstThing(map, ThingDefOf.Hive) == null && x.GetFirstThing(map, ThingDefOf.TunnelHiveSpawner) == null);
            return true;
        }
        ...
        private static void CalculateLocationCandidates(Map map)
        {
            ...
            for (int i = 0; i < map.Size.z; i++)
            {
                for (int j = 0; j < map.Size.x; j++)
                {
                    ...
                    float scoreAt = XenomorphHiveCellFinder.GetScoreAt(cell, map);
                    if (scoreAt > 0f)
                    {
