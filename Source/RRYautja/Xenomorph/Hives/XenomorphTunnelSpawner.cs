...
namespace RimWorld
{
    public class XenomorphTunnelSpawnerExtension : DefModExtension
    {
    ...
    */
    ...
    public class XenomorphTunnelSpawner : ThingWithComps
    {
        public XenomorphTunnelSpawnerExtension Ext => this.def.HasModExtension<XenomorphTunnelSpawnerExtension>() ? this.def.GetModExtension<XenomorphTunnelSpawnerExtension>() : null;
        /*
        ...
        */
        ...
        public Faction faction
        {
            get
            {
                ...
                {
                    ThingDef def_HiveLike = hiveDef;
                    faction = Find.FactionManager.FirstFactionOfDef(def_HiveLike.GetModExtension<XenomorphHiveExtension>().Faction);
                }
                else
                {
            ...
            get
            {
                ...
                else
                {
                    if (!(from def in DefDatabase<ThingDef>.AllDefs
                          where (def.HasModExtension<XenomorphHiveExtension>() && (def.GetModExtension<XenomorphHiveExtension>().Faction == faction.def && def.GetModExtension<XenomorphHiveExtension>().Faction != null))
                          select def).TryRandomElement(out ThingDef var))
                    {
                        thing = var;
                    }
                    else
        ...
        public List<PawnKindDef> pawnKinds
        {
            get
            {
                ...
                if (Ext.PawnKinds != null && Ext.PawnKinds.Count>0)
                {
                ...
                {
                    ThingDef def_HiveLike = hiveDef;
                    if (def_HiveLike.GetModExtension<XenomorphHiveExtension>().PawnKinds != null && def_HiveLike.GetModExtension<XenomorphHiveExtension>().PawnKinds.Count > 0)
                    {
                        pawnKinds = def_HiveLike.GetModExtension<XenomorphHiveExtension>().PawnKinds;
                    }
        ...
        public override void Tick()
        {
            if (base.Spawned)
            {
                ...
                ResetStaticData();
                if (Rand.MTBEventOccurs(FilthSpawnMTB, 1f, 1.TicksToSeconds()) && CellFinder.TryFindRandomReachableCellNear(base.Position, base.Map, XenomorphTunnelSpawner.FilthSpawnRadius, TraverseParms.For(TraverseMode.NoPassClosedDoors, Danger.Deadly, false), null, null, out IntVec3 c, 999999))
                {
                ...
                if (this.secondarySpawnTick <= Find.TickManager.TicksGame)
                {
                    ...
                    if (this.spawnHive)
                    {
                        ...
                        hive = (XenomorphHive)GenSpawn.Spawn(ThingMaker.MakeThing(hiveDef, null), position, map, WipeMode.Vanish);
