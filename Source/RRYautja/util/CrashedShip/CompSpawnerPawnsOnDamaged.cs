            ...
            try
            {
                while (this.pointsLeft > 0f)
                {
                    if (!(from def in DefDatabase<PawnKindDef>.AllDefs
                          where ((def.defaultFactionType == faction.def && def.defaultFactionType != null) || (def.defaultFactionType == null && faction.def.pawnGroupMakers.Any(pgm => pgm.options.Any(opt => opt.kind == def) && pgm.kindDef != PawnGroupKindDefOf.Trader && pgm.kindDef != PawnGroupKindDefOf.Peaceful))) && def.isFighter && def.combatPower <= this.pointsLeft && (SettingsHelper.latest.AllowPredaliens || def != XenomorphDefOf.AvP_Xenomorph_Predalien) && def != XenomorphDefOf.AvP_Xenomorph_Thrumbomorph
