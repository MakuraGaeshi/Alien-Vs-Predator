    ...
    public class HediffGiver_Synth : HediffGiver
    {
        ...
        public override void OnIntervalPassed(Pawn pawn, Hediff cause) 
        {
            if (pawn.def == USCMDefOf.RRY_Synth)
            {
                ...
                BodyPartRecord record = pawn.RaceProps.body.AllParts.Find(x => x.def.defName == "RRY_BehavorialInhibitor");
                ...
                bool has = (pawn.health.hediffSet.HasHediff(USCMDefOf.RRY_Damaged_Inhibitor) && pawn.health.hediffSet.HasHediff(USCMDefOf.RRY_Defective_Inhibitor));
                ...
                if (Rand.MTBEventOccurs(this.mtbDays, 60000f, 60f) && Rand.Chance(chance) && base.TryApply(pawn, null))
                {
                    ...
                    if (!has)
                    {
                        pawn.health.AddHediff(USCMDefOf.RRY_Damaged_Inhibitor, record);
                    }
                    */
                    pawn.mindState.mentalStateHandler.TryStartMentalState(MentalStateDefOf.Berserk);
