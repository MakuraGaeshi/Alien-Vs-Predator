...
namespace RimWorld
{
    ...
    public class ThinkNode_ConditionalNotPawnKind : ThinkNode_Conditional
    {
        ...
        {
            if (Find.Selector.SelectedObjects.Contains(pawn) && Prefs.DevMode && DebugSettings.godMode) Log.Message(string.Format("{0} needs {3} > {2}, Result: {1}", this, pawn.kindDef != this.pawnKind, pawn.kindDef, pawnKind));
